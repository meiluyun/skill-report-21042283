...
class NetWork:
    def __init__(self):
        self.input  = np.array([0 for _ in range(8)])
        self.l2     = np.array([0 for _ in range(4)])
        self.l3     = np.array([0 for _ in range(4)])
        self.output = np.array([0 for _ in range(4)])
...
if __name__ == '__main__':
    main1(int(input("evolution：")), int(input("Show training process？（1/0）")))
    main2()
    main3()
